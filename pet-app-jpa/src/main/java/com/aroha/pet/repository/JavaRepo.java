...
public interface JavaRepo extends JpaRepository<JavaPojo, Long>{
	 @Query(value="SELECT\r\n" +
			    ...
			    "        CAST(((COUNT(Distinct(q.scenario)))/(count(q.javastr)))*100 as decimal(5,\r\n" +
			    "        2))as productivity       \r\n" +
			    "    FROM\r\n" +
			    "        users u      \r\n" +
			    "    LEFT JOIN\r\n" +
			    ...
			    @Query(value="select * from java_pojo where created_at=?1 and question_id=?2",nativeQuery = true)
			    public JavaPojo searchJavaRepo(String createdAt,int questionId);
