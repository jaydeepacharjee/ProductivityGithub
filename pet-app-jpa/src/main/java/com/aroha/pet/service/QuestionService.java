...
public class QuestionService {
	...
	public List<QuestionDataRequest> getQuestionData(int scenarioId){
		...
		while(itr.hasNext()) {
			...
			if(question.getScenario().getScenarioId() != scenarioId) continue;
	...
	//        Update Question by admin and mentor
/*
	public String checkQuestion(Question ques) {
		int id=ques.getQuestionId();
		Optional<Question> question=questionRepository.findById(id);		
		if(!question.isPresent()) {
			throw new ResourceNotFoundException("Question with id "+id+" not Exist");
		}
		Question obj=question.get();
		obj.setQuestionDesc(ques.getQuestionDesc());
		try {
			questionRepository.save(obj);
			logger.info("Question updated");
		}catch(Exception ex) {
			logger.error("Error updating question "+ex.getMessage());
			return ex.getMessage();
		}
